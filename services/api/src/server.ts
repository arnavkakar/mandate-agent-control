...
app.post("/v1/auth/login", async (request, reply) => {
  ...
  if (!organization) return reply.code(403).send({ error: "WORKSPACE_NOT_FOUND" });
  return { token: await issueToken(user.id, membership.organizationId), user: { id: user.id, name: user.name, email: user.email }, organization };
});
app.get("/v1/me", async request => {
  const auth = await human(request);
  const [user] = await db.select({ id: users.id, name: users.name, email: users.email }).from(users).where(eq(users.id, auth.userId)).limit(1);
  const [organization] = await db.select().from(organizations).where(eq(organizations.id, auth.organizationId)).limit(1);
...
app.get("/v1/agents", async request => {
  ...
  return db.select({
    id: agents.id, name: agents.name, purpose: agents.purpose, status: agents.status, createdAt: agents.createdAt,
    spentThisMonthCents: dsql<number>`(select coalesce(sum(entry.amount_cents), 0) from budget_ledger_entries entry where entry.agent_id = ${agents.id} and entry.period = ${period})`,
    mandate: dsql<MandatePolicy | null>`(select active_mandate.policy from mandates active_mandate where active_mandate.agent_id = ${agents.id} and active_mandate.active = true order by active_mandate.version desc limit 1)`,
  }).from(agents)
...
app.post("/v1/agents", async (request, reply) => {
  ...
  await appendAudit(db, { organizationId: auth.organizationId, eventType: "AGENT_CREATED", actorType: "USER", actorId: auth.userId, subjectType: "AGENT", subjectId: agent.id, payload: input });
...
app.patch("/v1/agents/:id", async (request, reply) => {
  ...
  await appendAudit(db, { organizationId: auth.organizationId, eventType: "AGENT_UPDATED", actorType: "USER", actorId: auth.userId, subjectType: "AGENT", subjectId: agent.id, payload: input });
  return agent;
});
...
app.get("/v1/agents/:id/keys", async (request, reply) => {
  ...
  if (!agent) return reply.code(404).send({ error: "AGENT_NOT_FOUND" });
  return db.select({ id: apiKeys.id, name: apiKeys.name, prefix: apiKeys.prefix, scopes: apiKeys.scopes, lastUsedAt: apiKeys.lastUsedAt, revokedAt: apiKeys.revokedAt, createdAt: apiKeys.createdAt }).from(apiKeys).where(and(eq(apiKeys.agentId, id), eq(apiKeys.organizationId, auth.organizationId))).orderBy(desc(apiKeys.createdAt));
});
...
app.post("/v1/agents/:id/mandates", async (request, reply) => {
  ...
  const mandate = await db.transaction(async tx => {
    ...
    const [current] = await tx.select({ version: mandates.version }).from(mandates).where(and(eq(mandates.agentId, id), eq(mandates.organizationId, auth.organizationId))).orderBy(desc(mandates.version)).limit(1);
    await tx.update(mandates).set({ active: false }).where(and(eq(mandates.agentId, id), eq(mandates.organizationId, auth.organizationId)));
    const [created] = await tx.insert(mandates).values({ organizationId: auth.organizationId, agentId: id, version: (current?.version ?? 0) + 1, userIntent: input.userIntent, policy: input.policy, createdBy: auth.userId }).returning();
    await appendAudit(tx, { organizationId: auth.organizationId, eventType: "MANDATE_ACTIVATED", actorType: "USER", actorId: auth.userId, subjectType: "MANDATE", subjectId: created.id, payload: { agentId: id, version: created.version, userIntent: input.userIntent } });
    return created;
  });
  return reply.code(201).send(mandate);
});
async function authorizeAgent(agentId:string,organizationId:string,input:z.infer<typeof authRequestSchema>){
  const amountCents=Math.round(input.amount*100);
  return db.transaction(async tx=>{await tx.execute(dsql`select pg_advisory_xact_lock(hashtext(${agentId}))`);const [existing]=await tx.select().from(transactions).where(and(eq(transactions.agentId,agentId),eq(transactions.idempotencyKey,input.idempotencyKey))).limit(1);if(existing){const [decision]=await tx.select().from(authorizationDecisions).where(eq(authorizationDecisions.transactionId,existing.id)).limit(1);return {transaction:existing,decision,replayed:true}}
    const [agent]=await tx.select().from(agents).where(and(eq(agents.id,agentId),eq(agents.organizationId,organizationId))).limit(1);const [mandate]=await tx.select().from(mandates).where(and(eq(mandates.agentId,agentId),eq(mandates.active,true))).orderBy(dsql`${mandates.version} desc`).limit(1);if(!agent||!mandate)throw new Error("NO_ACTIVE_MANDATE");
    const period=new Date().toISOString().slice(0,7);const [spend]=await tx.select({total:dsql<number>`coalesce(sum(${budgetLedger.amountCents}), 0)`}).from(budgetLedger).where(and(eq(budgetLedger.agentId,agent.id),eq(budgetLedger.period,period)));const [history]=await tx.select({count:dsql<number>`count(*)`,average:dsql<number>`coalesce(avg(${transactions.amountCents}), 0)`}).from(transactions).where(eq(transactions.agentId,agent.id));const [known]=await tx.select({count:dsql<number>`count(*)`}).from(transactions).where(and(eq(transactions.agentId,agent.id),dsql`lower(${transactions.merchant}) = lower(${input.merchant})`));const [velocity]=await tx.select({count:dsql<number>`count(*)`}).from(transactions).where(and(eq(transactions.agentId,agent.id),dsql`${transactions.createdAt} > now() - interval '1 hour'`));const evaluationInput={agentStatus:agent.status,amountCents,merchant:input.merchant,category:input.category,country:input.country,isNewMerchant:Number(known.count)===0,spentThisMonthCents:Number(spend.total),transactionCountLastHour:Number(velocity.count),averageTransactionCents:Number(history.average),policy:mandate.policy};const policy=evaluatePolicy(evaluationInput);const risk=evaluateRisk(evaluationInput);
    const [transaction]=await tx.insert(transactions).values({organizationId,agentId,idempotencyKey:input.idempotencyKey,amountCents,merchant:input.merchant,category:input.category,country:input.country.toUpperCase(),currency:input.currency.toUpperCase(),metadata:input.metadata}).returning();const [decision]=await tx.insert(authorizationDecisions).values({organizationId,transactionId:transaction.id,mandateId:mandate.id,decision:policy.decision,policyRules:policy.rules,reasons:policy.reasons,riskScore:risk.score,riskFactors:risk.factors,engineVersion:"policy-v1.0.0"}).returning();if(policy.decision==="APPROVED")await tx.insert(budgetLedger).values({organizationId,agentId,transactionId:transaction.id,amountCents,kind:"AUTHORIZED",period});if(policy.decision==="APPROVAL_REQUIRED")await tx.insert(approvalRequests).values({organizationId,decisionId:decision.id,expiresAt:new Date(Date.now()+86_400_000)});await appendAudit(tx,{organizationId,eventType:"AUTHORIZATION_DECIDED",actorType:"AGENT",actorId:agentId,subjectType:"TRANSACTION",subjectId:transaction.id,payload:{decision:policy.decision,reasons:policy.reasons,risk}});return {transaction,decision,replayed:false};
  });
}
app.post("/v1/authorization-requests", async (request, reply) => {
  ...
  try {
    const output = await authorizeAgent(key.agentId,key.organizationId,input);await db.update(apiKeys).set({lastUsedAt:new Date()}).where(eq(apiKeys.id,key.id));
...
app.post("/v1/simulator/authorization-requests",async(request,reply)=>{const auth=await human(request);const input=z.object({agentId:z.string().uuid(),request:authRequestSchema}).parse(request.body);try{return reply.code(201).send(await authorizeAgent(input.agentId,auth.organizationId,input.request))}catch(error){if(error instanceof Error&&error.message==="NO_ACTIVE_MANDATE")return reply.code(409).send({error:"NO_ACTIVE_MANDATE"});throw error}});
app.post("/v1/demo-seed",async(request,reply)=>{const auth=await human(request);const [existing]=await db.select({count:dsql<number>`count(*)`}).from(agents).where(eq(agents.organizationId,auth.organizationId));if(Number(existing.count)>0)return reply.code(409).send({error:"WORKSPACE_NOT_EMPTY"});
  const definitions=[
    {name:"Procurement Agent",purpose:"Software and office purchasing",policy:{monthlyBudgetCents:200000,maxTransactionCents:25000,approvalThresholdCents:25000,allowedCategories:["software","office equipment"],blockedCategories:["crypto","gambling"],allowedMerchants:["Notion","AWS"],blockedMerchants:["Binance"],allowedCountries:["US"],requireApprovalForNewMerchant:true,requireApprovalForAll:false,expiresAt:null}},
    {name:"Travel Coordinator",purpose:"Team travel and accommodation",policy:{monthlyBudgetCents:300000,maxTransactionCents:30000,approvalThresholdCents:30000,allowedCategories:["travel"],blockedCategories:["crypto","gambling"],allowedMerchants:["Delta"],blockedMerchants:[],allowedCountries:["US"],requireApprovalForNewMerchant:true,requireApprovalForAll:false,expiresAt:null}},
  ] satisfies Array<{name:string;purpose:string;policy:MandatePolicy}>;
  const created=await db.transaction(async tx=>{const rows=[];for(const definition of definitions){const [agent]=await tx.insert(agents).values({organizationId:auth.organizationId,name:definition.name,purpose:definition.purpose}).returning();await tx.insert(mandates).values({organizationId:auth.organizationId,agentId:agent.id,version:1,userIntent:`Demo mandate for ${definition.purpose}.`,policy:definition.policy,createdBy:auth.userId});await appendAudit(tx,{organizationId:auth.organizationId,eventType:"DEMO_AGENT_CREATED",actorType:"USER",actorId:auth.userId,subjectType:"AGENT",subjectId:agent.id,payload:{name:agent.name,synthetic:true}});rows.push(agent)}return rows});
  const samples=[{agentId:created[0].id,idempotencyKey:`demo-notion-${Date.now()}`,amount:96,merchant:"Notion",category:"software",country:"US"},{agentId:created[0].id,idempotencyKey:`demo-apple-${Date.now()}`,amount:899,merchant:"Apple",category:"office equipment",country:"US"},{agentId:created[0].id,idempotencyKey:`demo-binance-${Date.now()}`,amount:600,merchant:"Binance",category:"crypto",country:"KY"},{agentId:created[1].id,idempotencyKey:`demo-delta-${Date.now()}`,amount:389,merchant:"Delta",category:"travel",country:"US"}];for(const sample of samples){await authorizeAgent(sample.agentId,auth.organizationId,{...sample,currency:"USD",metadata:{syntheticDemo:true}})}
  await appendAudit(db,{organizationId:auth.organizationId,eventType:"DEMO_WORKSPACE_SEEDED",actorType:"USER",actorId:auth.userId,subjectType:"ORGANIZATION",subjectId:auth.organizationId,payload:{synthetic:true,agents:created.length,transactions:samples.length}});return reply.code(201).send({agents:created.length,transactions:samples.length,synthetic:true})});
app.get("/v1/transactions", async request => {
  const auth = await human(request);
  return db.select({
    id: transactions.id, merchant: transactions.merchant, category: transactions.category, country: transactions.country, currency: transactions.currency, amountCents: transactions.amountCents, metadata: transactions.metadata, createdAt: transactions.createdAt,
    agentId: agents.id, agentName: agents.name, decisionId: authorizationDecisions.id, decision: authorizationDecisions.decision, reasons: authorizationDecisions.reasons, policyRules: authorizationDecisions.policyRules, riskScore: authorizationDecisions.riskScore, riskFactors: authorizationDecisions.riskFactors, engineVersion: authorizationDecisions.engineVersion,
    approvalRequestId: approvalRequests.id, approvalStatus: approvalRequests.status,
  }).from(transactions)
    .innerJoin(agents, eq(agents.id, transactions.agentId))
    .innerJoin(authorizationDecisions, eq(authorizationDecisions.transactionId, transactions.id))
    .leftJoin(approvalRequests, eq(approvalRequests.decisionId, authorizationDecisions.id))
    .where(eq(transactions.organizationId, auth.organizationId)).orderBy(desc(transactions.createdAt)).limit(250);
});
app.get("/v1/approval-requests", async request => {
  const auth = await human(request);
  return db.select({
    id: approvalRequests.id, status: approvalRequests.status, expiresAt: approvalRequests.expiresAt, createdAt: approvalRequests.createdAt, resolvedAt: approvalRequests.resolvedAt, resolutionNote: approvalRequests.resolutionNote,
    decisionId: authorizationDecisions.id, decision: authorizationDecisions.decision, reasons: authorizationDecisions.reasons, policyRules: authorizationDecisions.policyRules, riskScore: authorizationDecisions.riskScore, riskFactors: authorizationDecisions.riskFactors,
    transactionId: transactions.id, merchant: transactions.merchant, category: transactions.category, country: transactions.country, currency: transactions.currency, amountCents: transactions.amountCents,
    agentId: agents.id, agentName: agents.name,
  }).from(approvalRequests)
    .innerJoin(authorizationDecisions, eq(authorizationDecisions.id, approvalRequests.decisionId))
    .innerJoin(transactions, eq(transactions.id, authorizationDecisions.transactionId))
    .innerJoin(agents, eq(agents.id, transactions.agentId))
    .where(eq(approvalRequests.organizationId, auth.organizationId)).orderBy(desc(approvalRequests.createdAt));
});
app.get("/v1/dashboard", async request => {
  const auth = await human(request); const period = new Date().toISOString().slice(0, 7);
  const [budget] = await db.select({ authorizedSpendCents: dsql<number>`coalesce(sum(${budgetLedger.amountCents}), 0)` }).from(budgetLedger).where(and(eq(budgetLedger.organizationId, auth.organizationId), eq(budgetLedger.period, period)));
  const [counts] = await db.select({ total: dsql<number>`count(*)`, approved: dsql<number>`count(*) filter (where ${authorizationDecisions.decision} = 'APPROVED')`, declined: dsql<number>`count(*) filter (where ${authorizationDecisions.decision} = 'DECLINED')`, review: dsql<number>`count(*) filter (where ${authorizationDecisions.decision} = 'APPROVAL_REQUIRED')`, highRisk: dsql<number>`count(*) filter (where ${authorizationDecisions.riskScore} >= 70)` }).from(authorizationDecisions).where(eq(authorizationDecisions.organizationId, auth.organizationId));
  const activeMandates = await db.select({ policy: mandates.policy }).from(mandates).where(and(eq(mandates.organizationId, auth.organizationId), eq(mandates.active, true)));
  const authorizedBudgetCents = activeMandates.reduce((sum, item) => sum + item.policy.monthlyBudgetCents, 0);
