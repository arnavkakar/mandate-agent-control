...
import {
  ...
  WalletCards, X, XCircle, Zap, Plug, KeyRound, Webhook, Copy, TerminalSquare, Database, Eye, EyeOff,
} from "lucide-react";
...
function AuthPage({onAuthenticated}:{onAuthenticated:(session:Session)=>void}){
  ...
  return <main className="auth-shell"><section className="auth-card card"><AppLogo/><span className="kicker">PRODUCTION CONTROL CENTER</span><h1>{mode==="login"?"Sign in to Mandate":"Create your workspace"}</h1><p>Manage deterministic authorization for every agent purchase request.</p><form onSubmit={submit}>{mode==="signup"&&<><label>Your name<input required name="name" minLength={2} autoComplete="name"/></label><label>Organization<input required name="organizationName" minLength={2}/></label></>}<label>Email<input required name="email" type="email" autoComplete="email"/></label><label>Password<span className="password-field"><input required name="password" type={passwordVisible?"text":"password"} minLength={mode==="signup"?12:1} autoComplete={mode==="login"?"current-password":"new-password"}/><button type="button" className="password-toggle" aria-label={passwordVisible?"Hide password":"Show password"} aria-pressed={passwordVisible} onClick={()=>setPasswordVisible(value=>!value)}>{passwordVisible?<EyeOff aria-hidden="true" size={18}/>:<Eye aria-hidden="true" size={18}/>}</button></span></label>{error&&<div className="auth-error" role="alert">{error}</div>}<button className="primary full" disabled={busy}>{busy?"Please wait…":mode==="login"?"Sign in":"Create workspace"}</button></form><button className="text-button auth-switch" onClick={()=>{setMode(mode==="login"?"signup":"login");setPasswordVisible(false);setError("")}}>{mode==="login"?"New to Mandate? Create a workspace":"Already have an account? Sign in"}</button><small>No real payments are processed. Mandate stores authorization intent and decisions only.</small></section></main>;
}
